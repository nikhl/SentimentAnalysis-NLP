...
import os
target_dir = r"/home/nikhil/workspace/encodeText/newimdb/unlabeled"
vocabFilename = os.path.join(target_dir,'imdb_test_vocabulary.txt')
trainLabelsFilename = os.path.join(target_dir,'imdb_test.labels')
trainDataFilename = os.path.join(target_dir,'imdb_test.data')
stopfile = os.path.join(target_dir,'stop.words')
trainvocabFilename = os.path.join(r"/home/nikhil/workspace/encodeText/newimdb/labeled",'imdb_train_vocabulary.txt')

train_vocab = open(trainvocabFilename,'r').read().split()
...
def loaddata():
    ...
    file = open('newimdb/unlabeled_reviews.xml','r');
    handler = file.read()
    ...
    for each in reviews:
        ...
        for word in words:
            if word in train_vocab:
                row = str(i+1)+" "
                word_index = train_vocab.index(word)
